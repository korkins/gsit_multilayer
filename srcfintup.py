...
#
def srcfintup(m, mu, mu0, Ig05, mug, wg, ng2, dtau, mask, nlr, nlv, ssa, xk, nol, nk, Igboa, srfa):
    '''
    Task:
        To compute m-th Fourier moment of 2+ scattering intensity, I2m, at a
        user-defined upward direction mu and at many atmopsheric levels.
    In:
        m      i             Fourier moment; m = 0, 1, ... nm-1
        mu     f             cos(vza_up) < 0: up
        ...
        Igboa  f[ng1]        m-th Fourier moment intensity at BOA
        srfa   f             Lambertian surface albedo
    Out:
        Iup   f[nlv]         2+ scattering orders solution at all levels downward
    Notes:
    ...
    if m == 0 and srfa > srfa_min:
        I2up[nlv-1] = 2.0 * srfa * np.dot(Igboa, mug[ng1:ng2] * wg[ng1:ng2]) # no direct sunbeam bouncing
    
    for ilv in range(nlv-2, -1, -1):
        iol = mask[ilv]
        Tup = np.exp(dtau[ilv]/mu) # mu < 0: up
        J = 0.5 * ssa[iol] * np.dot(wpij[iol, :], Ig05[ilv])
        I2up[ilv] = I2up[ilv+1] * Tup + (1.0 - Tup) * J # note: SS is neither added nor subtracted
    # end: for ilv up
...
#       and combined optical thicknesses, and combined single scattering albedo
    ...
    for m in range(nm):
        ...
        Igboa = Im_dn_sfi[nlv-1, :, m]
        
        for imu in range(nup):
            mu = mu_up[imu]
            I2upm = srcfintup(m, mu, mu0, Img05, mug, wg, ng2, dtau, mask, nlr, nlv, ssa, xk_multiscat, nol, nk, Igboa, srfa)
            for ilv in range(nlv):
                I2up[ilv, imu, :] += I2upm[ilv] * kron_cmaz
        deltm0 = 2.0        
        print(f"m = {m: d}")
    ...
    Iup_toa = I1up[0, :, :] + I2up[0, :, :]
    rerr = 100.0*np.abs(Iup_toa/I_bmrk_toa - 1.0)
    emax = np.amax(rerr)
